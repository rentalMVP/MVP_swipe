...
const GalleryItem = ({ fields, index, id }) => {
    ...
    return (
        <div className="card" aria-haspopup="true">
            ...
            <Link href={`/cards?query=${fields["Item ID"]}`}><p className="itemName" style={{ color:"white"}}>{fields["Item Name"]}</p></Link>
            <Link href={`/rental_owner?query=${fields["Rental Owner"][0]}`}><p className="text-white rentalOwnerName hover:underline">{
                rentalOwenrName
            }</p></Link>
            <p className="text-white itemPrice">{fields["Item Price"]}</p>
        </div>
    )
